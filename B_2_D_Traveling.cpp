...
void solve()
{
    int n, major, start, end;
    cin >> n >> major >> start >> end;
    vl x(n + 1), y(n + 1);
    for (int i = 1; i <= n; i++)
    {
        cin >> x[i] >> y[i];
    ...
    ll ans = abs(x[start] - x[end]) + abs(y[start] - y[end]);
    ll mins = ll(1e10), mint = ll(1e10);

    for (int i = 1; i <= major; i++)
    {
        mins = min(mins, abs(x[start] - x[i]) + abs(y[start] - y[i]));
        mint = min(mint, abs(x[end] - x[i]) + abs(y[end] - y[i]));
    }
