...
//from ds for learning difference of red  black tree(set) and Unordered_Set - Buckets Based on Hash mar 25-2026
struct custom_hash {
...
void solve()
{
    ...
    unordered_set<ll, custom_hash> st(v.begin(), v.end());

    each(x, st)
    {
        if (st.count(x + k))
        {
            //cout<<st.count(x + k)<<endl;
            f = 1;
            break;
        }
    }
